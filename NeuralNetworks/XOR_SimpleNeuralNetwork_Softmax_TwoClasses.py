...
Evaluate the performance of a simple neural network (with a 3 neuron hidden layer
 and softmax final layer) applied to an XOR type dataset. Unlike logsitic regression,
this simple neural network does not need a calculated X1*X2 feature for the XOR dataset.
However, the training takes a lot longer than logistic regression.
...
class SimpleNeuralNetwork(object):
  ...
  def grads(self, X, y_target_onehot, y_hat, L2):
    ...
    # For sigmoid activation (g(x)' = A1*(1-A1))
  ...
  def fit(self, X, y, epochs=100000, alpha=0.1, L2=0.0):
...
if __name__ == '__main__':
  ...
  costs = model.fit(X,y,epochs=200000)
